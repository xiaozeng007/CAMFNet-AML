...
class MetricsTop():
    def __init__(self, train_mode):
        if train_mode == "regression":
            self.metrics_dict = {
                ...
                'MUSTARDPP': self.__eval_mustard_regression,
                'SARCNET': self.__eval_mustard_regression
            }
        else:
            self.metrics_dict = {
                ...
                'MUSTARD': self.__eval_mustard_classification,
                'MUSTARDPP': self.__eval_mustard_classification,
                'SARCNET': self.__eval_mustard_classification
            }
    ...
    def __eval_mustard_regression(self, y_pred, y_true):
        """
        MUStARD sarcasm detection evaluation (binary: sarcastic vs non-sarcastic)
        Labels: -1.0 (non-sarcastic) and 1.0 (sarcastic)
        """
        ...
        # MAE and Correlation for regression
    ...
    def __eval_mustard_classification(self, y_pred, y_true):
        """
        MUStARD sarcasm detection evaluation (classification mode)
        Binary classification: non-sarcastic (0) vs sarcastic (1)
        """
        y_pred = y_pred.cpu().detach().numpy()
        y_true = y_true.cpu().detach().numpy()
