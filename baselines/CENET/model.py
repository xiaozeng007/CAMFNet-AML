...
# Import prune_linear_layer compatibly across transformers versions
try:
    from transformers.pytorch_utils import prune_linear_layer  # newer versions
except Exception:
    # Fallback implementation compatible with basic linear pruning
    def prune_linear_layer(layer, index, dim=0):
        """Prune a linear layer (a model parameters) to keep only entries in index.

        Args:
            layer (nn.Linear): The layer to prune.
            index (torch.LongTensor): Indices to keep.
            dim (int): Dimension along which to prune (0 for output neurons, 1 for input features).
        Returns:
            nn.Linear: New pruned layer with weights and bias (if any) sliced accordingly.
        """
        if not isinstance(layer, nn.Linear):
            raise TypeError("prune_linear_layer expects an nn.Linear layer")
        if dim == 0:
            # Prune output neurons (rows)
            new_out_features = index.size(0)
            new_layer = nn.Linear(layer.in_features, new_out_features, bias=layer.bias is not None)
            new_layer.weight.data = layer.weight.data[index, :].clone()
            if layer.bias is not None:
                new_layer.bias.data = layer.bias.data[index].clone()
            return new_layer
        elif dim == 1:
            # Prune input features (columns)
            new_in_features = index.size(0)
            ...
            raise ValueError("Invalid dim for prune_linear_layer: expected 0 or 1")
...
class BertPreTrainedModel(PreTrainedModel):
    ...
    # pretrained_model_archive_map = BERT_PRETRAINED_MODEL_ARCHIVE_MAP  # 已在新版本transformers中移除
